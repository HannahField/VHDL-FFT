...
-- The implementation is streaming, as it is intended to be used in OFDM and SC-FDMA.
...
-- Ping-pong style RAM
-- Storing a complex value as the concat between real and imag
...
begin
-- Ensure N <= 4096 at compile time

	assert logN <= 12
	report "FFT: N exceeds twiddle table size"
	severity failure;

-- Generate logN stages
		...
		if rising_edge(CLK) then
			if (RST = '1') then
				...
				if (VALID_IN = '1') then
					...
					-- If there is incoming data, write it to the current write-ram
				...
				-- If the current read ram has valid data, then send it to the first stage
				if (VALID_WRT = '1') then
				...
				-- If the output of the last stage is valid, then output that
