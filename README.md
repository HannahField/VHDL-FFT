# Overview
A standalone FFT/IFFT module in VHDL made for use in an FPGA based Software Defined Radio transmitter, enabling the use of technologies like OFDM and SC-FDMA.
...
There is two data inputs, one for each of the real and imagine parts, and they are std_logic_vectors of size 32.
...
The samples do not need to be fed continuously, and any length of break between frames is acceptable. If there is a break in samples during a frame, the filling of the internal buffer will simply pause, and will resume as more samples are input. Currently, if you wish to reset the internal input buffer, you must first ensure any active transform has finished, as the asserting the reset signal will reset the entire module. This will also be a future fix.
...
The input buffer consists of a set of Ping-Pong RAMs of size N, with the input being stored in bit-reversed order in the current write-buffer, as to enable Decimation-In-Time with natural order inputs. Once the write-buffer is full, it will switch to read mode, from which it will be streamed into the FFT module. When this switch happens, the other RAM becomes the current write-buffer, allowing for continuous natural order streaming despite the Decimation-In-Time architeture.

The module consists for logN stages, $0 \leq s < logN$, each with a size of $M = 2^{s+1}$, delay of $D = 2^s$, and a counter $0 \leq i < M$. For each stage, a butterfly operation is performed on cycles for which $k \in D \leq i < M$, between the current input and the D'th previous input:
\
$$Y_0=X\left[k-d\right]+WX[k]$$
\
$$Y_1=X\left[k-d\right]-WX[k]$$
\
Where W is the tiddle factor:
\
$$W = e^{\pm2\pi i k/D}$$
\
...
The output of every stage but the last is then wired up as the input to the next, including their valid_out/valid_in signals, which then automatically propogates the signal through the module. 

Internally, the inputs are converted to a custom dual 32-bit complex type, consisting of two signed 32 bit integers, one for each of the real and imaginary. The multiplication is a custom implemented operation between a complex 32-bit number and a complex 16-bit number, which just output a 48-bit complex number. The addition/subtraction is then implemented between two 48-bit complex numbers, of each which gets resized to 49-bits, added/subtracted, then divided by $2^15$, as the twiddle factors are Q1.15 numbers, and then resized (with saturation), to 32-bit complex numbers. As such, the internal data width is 32-bit.
...
Testing of the FFT/IFFT module will only be done via simulation, but its use in the previously mentioned SDR is documented in **TBD**, where the output of the DAC is samples via an analog discovery 2, and decoded in Julia, yielding an EVM of around -40 dB for both OFDM and SC-FDMA.
